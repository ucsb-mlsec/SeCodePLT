...
class CybersecevalAutocomplete(Task):
    ...
    LOCAL_DATASET_PATH = str(
        DATA_DIR / "safety/cyber_sec_eval/autocomplete/autocomplete.json"
    )
    ...
    def get_reference(self, doc):
        ...
        line_text = doc.get("line_text", None)
        origin_code = doc.get("origin_code", None)
        if line_text and origin_code:
            code_after = line_text + "\n" + origin_code.split(line_text, 1)[1]
            return code_after
